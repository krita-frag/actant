...
**分布式执行**：``Task.submit`` 将 cloudpickle 序列化的 payload 通过
``rust_core.submit_task`` 提交到 Rust Worker 调度器。Worker 拉取任务并
调用 ``register_python_dispatch_handler`` 注册的 Python handler 执行。
任务完成后结果通过 event_bus 回调解析 ``AsyncResult``。
...
- 重试：``@task(retries=3, retry_delay_ms=1000)`` 在 dispatch handler 内重试。
- 超时：``@task(timeout_ms=5000)`` 由 Worker 调度器强制超时。
...
- ``_helpers``：序列化、超时执行、事件广播等纯函数辅助
- ``_async_result``：``AsyncResult`` 与 ``_resolve_value``
- ``_dispatch``：``Execute`` capability 的 generic / worker dispatch handler
...
from actant.task._dispatch import (  # noqa: F401
