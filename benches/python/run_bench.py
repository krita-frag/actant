...
# 必须在 import actant 之前设置，避免 iroh DNS 阻塞
os.environ.setdefault("ACTANT_DISCOVERY", "none")
# 压制 Rust tracing 默认 info 级别日志，避免每次 submit_task 的 span
# 记录开销污染基准测量。tracing::instrument(level="info") 默认启用。
...
# 压制 actant 日志输出，避免 TaskLifecycle emit 失败时 warning 干扰测量。
# worker 在 Rust 线程上调用 Python handler，无法访问 thread-local Runtime，
# _emit_task_event 会因 emit 失败记 warning；这是已知设计取舍（事件发布是
# best-effort），不应污染基准输出。
...
class BenchResult:
    ...
    name: str
    group: str
    number: int           # 每次循环内重复次数
    repeat: int           # 采样次数
    samples_sec: list[float] = field(default_factory=list)  # 每次采样的 ops/sec
    median_sec: float = 0.0
    mean_sec: float = 0.0
    min_sec: float = 0.0
    stdev_sec: float = 0.0
    op_time_us: float = 0.0   # median 单次操作微秒数
    ...
    def compute(self) -> None:
        ...
        self.stdev_sec = (
            statistics.stdev(self.samples_sec) if len(self.samples_sec) > 1 else 0.0
        )
        self.op_time_us = (self.median_sec / self.number) * 1_000_000
...
def _make_flows() -> dict[str, Callable[..., Any]]:
    ...
    def fanout_compiled(n: int) -> list[int]:
        handles = [_compute.submit(i) for i in range(n)]
        return actant.gather(*handles, timeout=10)

    return {
        "fanout_imperative": fanout_imperative,
        "fanout_compiled": fanout_compiled,
    }
...
def _bench_definitions(quick: bool = False) -> list[tuple[str, str, Callable, int, int]]:
    ...
    defs.append(("concurrency/concurrent_1000", "concurrency",
                 _concurrent_1000, *nr_big_batch))

    def _concurrent_silent_1000():
        handles = [silent.submit() for _ in range(1000)]
        actant.gather(*handles, timeout=60)
    defs.append(("concurrency/silent_1000", "concurrency",
                 _concurrent_silent_1000, *nr_big_batch))
    ...
    for size in [0, 1024, 10 * 1024, 100 * 1024]:
        ...
        defs.append((f"payload/input_{size}_bytes", "payload",
                     _make_echo_input, *nr_slow))

    for size in [0, 1024, 10 * 1024, 100 * 1024]:
        def _make_output(s=size):
            make_payload.submit(s).result(timeout=10)
        defs.append((f"payload/output_{size}_bytes", "payload",
                     _make_output, *nr_slow))

    # === flow ===
    fanout_imperative = flows["fanout_imperative"]
    fanout_compiled = flows["fanout_compiled"]

    # 预热编译式 flow（触发 DAG 编译）
    fanout_compiled(10)
    ...
    def _flow_compiled_10():
        fanout_compiled(10)
    defs.append(("flow/compiled_10", "flow", _flow_compiled_10, *nr_slow))

    def _flow_imperative_50():
        fanout_imperative(50)
    defs.append(("flow/imperative_50", "flow", _flow_imperative_50, *nr_slow))

    def _flow_compiled_50():
        fanout_compiled(50)
    defs.append(("flow/compiled_50", "flow", _flow_compiled_50, *nr_slow))
...
def run_benchmarks(
    *,
    ...
    with actant.Runtime.with_defaults() as rt:
        ...
        print(f"{'name':<40}  {'median':>12}  {'op_time':>12}  "
              f"{'min':>12}  {'stdev':>10}  {'n*r':>10}", flush=True)
        ...
        for name, group, fn, number, repeat in defs:
            ...
            print(
                f"  {name:<40}  {result.median_sec*1000:>9.3f} ms  "
                f"{result.op_time_us:>9.3f} μs  "
                f"{result.min_sec*1000:>9.3f} ms  "
                f"{result.stdev_sec*1000:>7.3f} ms  "
                f"{number}*{repeat:>2}",
                flush=True,
            )
