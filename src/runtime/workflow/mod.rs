...
//!
//! [`Orchestrator`](orchestrator::Orchestrator) 是工作流生命周期的中央协调器：
//! - **提交**：接收 [`Dag`](dag::Dag) 结构，通过 `Store` 持久化。
//! - **执行**：计算根任务、追踪待完成依赖，在前驱完成时产出可执行的
//!   [`TaskDefinition`](crate::common::TaskDefinition)。
//! - **完成**：工作流进入终态时通知等待者。
//! - **恢复**：[`Orchestrator::recover`](orchestrator::Orchestrator::recover)
...
//! | 模块 | 职责 |
//! |------|------|
//! | [`dag`](dag) | DAG 结构、边校验、拓扑查询；`WorkflowExecution` / `TaskState` |
//! | [`orchestrator`](orchestrator) | `Orchestrator` 状态机与事件溯源 |
//! | [`scheduler`](scheduler) | 任务调度策略（FIFO / 优先级） |
//! | [`failover`](failover) | 基于租约的 orchestrator 故障转移 |
//! | [`gossip`](gossip) | 基于 gossipsub 的 DAG 状态复制 |
//! | [`actor`](actor) | 上述组件的 Actor 化封装 |
//! | [`messaging`](messaging) | Actor 消息编解码 |
//! | [`runtime`](runtime) | `Worker` 执行循环 |
